...
namespace MWTrace_beta
{
    public partial class Ensamble_Etiquetado : Form
    {
        ...
        private bool Validate_WR11XGI()
        {
            bool isValidate = true;


            if (!modeloorden.Existe("select COUNT(*) from [digidb].[dbo].tb_Scan where sn = '" + txt_serial.Text.Trim() + "'"))
            {
                isValidate = false;
            }
            else if (!modeloorden.Existe("select COUNT(*) from [digidb].[dbo].tb_Scan where pcb like '" + txt_imei.Text.Trim() + "%'"))
            {
                isValidate = false;
            }
            else if (!modeloorden.Existe("select COUNT(*) from [digidb].[dbo].tb_Scan where mac = '" + txt_mac.Text.Trim() + "'"))
            {
                isValidate = false;
            }
            //else if (modeloorden.Existe("select COUNT(*) from [digidb].[dbo].tb_Scan where eui64 = '" + txt_serial.Text.Trim() + "'"))
            //{
        ...
        private void Ensamble_Etiquetado_Load(object sender, EventArgs e) => LoadForm();
        /// <summary>
        /// Validacion para la linea de SMART (COLD CHAIN) viejos o menores a D340000
        /// </summary>
        private bool ValidateOldSmart()
        {
            bool old = true;
            int n;
            string sn = txt_serial.Text.Trim().Substring(1, txt_serial.Text.Trim().Length - 1);
            bool isNumeric = int.TryParse(sn, out n);
            //txt_serial.Text.Trim().Length-1;
            if (isNumeric == true)
            {
                if (int.Parse(txt_serial.Text.Trim().Substring(1, txt_serial.Text.Trim().Length - 1)) >= 340000)
                {
                    old = false;
                }
            }
        ...
        private void Button1_Click(object sender, EventArgs e)
        {
            ...
            if (ValidarTextboxVacios() == true)
            {
                ...
                if (int.Parse(lbl_registrados.Text) < orden.Cantidad)
                {
                    ...
                    {
                        if (txt_caja.Enabled == true || ensamble.Tested != "Cold Chain" || modeloorden.Tested == 1)
                        {

                            if (txt_caja.Enabled == true || lbl_ensamble.Text.Substring(0, 4) == "WR11" || lbl_ensamble.Text.Substring(0, 3) == "XGI" && Validate_WR11XGI() == true)
                            {

                                if (txt_mac.Enabled == false || txt_mac.TextLength == 12 && txt_mac.Text.Substring(0, 3) == "000" || txt_mac.TextLength == 12 && txt_mac.Text.Substring(0, 2) == "00" || txt_mac.TextLength == 12 && txt_mac.Text.Substring(0, 1) == "0")
                                {
                                    ...
                                    if (txt_imei.Enabled == false || !(modeloorden.Existe("select COUNT(*) from tb_ModeloOrden where scanimei = '" + txt_imei.Text + "' and scanimei != ''")))
                                    {
                                        ...
                                        if (txt_mac.Enabled == false || !(modeloorden.Existe("select COUNT(*) from tb_ModeloOrden where scanmac = '" + txt_mac.Text + "' and scanmac != ''")))
                                        {
                                            ...
                                            {
                                                if (txt_caja.Enabled == true || ensamble.Tested != "Cold Chain" || ValidateOldSmart() == false)
                                                {
                                                    if (txt_partnumber.Text.Trim() == lbl_ensamble.Text.Trim() || txt_partnumber.Enabled == false)
                                                    {
                                                        if (ValidarEnsamble() == true)
                                                        {
                                                            ...
                                                            {
                                                                if (txt_caja.TextLength == 14 && txt_caja.Text.Substring(0, 2) == "MW" && int.Parse(txt_UnidadesCaja.Text) > 0)
                                                                {
                                                                    if (txt_modeloCaja.Text.Trim() == modeloorden.ReturnValue("select e.ensamble from tb_Orden o join tb_Ensamble e on o.id_ensamble = e.id_ensamble where id_orden = '" + orden.Id_orden + "'"))
                                                                    {
                                                                    ...
                                                                    {
                                                                        MessageBox.Show("Ensamble No Existe!", "ERROR DE CAJA!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                                                        txt_modeloCaja.Text = "";
                                                ...
                                                {
                                                    MessageBox.Show("El SERIAL NUMBER ES MENOR A D340000 !", "ERROR!");
                                                    Password();
                                                    TextboxToEmpty();
                                                }
                            ...
                            {
                                MessageBox.Show("Not Scan!", "FAIL!!!", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                                //Password();
        ...
        {
            //if (modeloorden.Tested != 0)
            //{

            //int passed = 0;
            if (!string.IsNullOrEmpty(txt_serial.Text.Trim()) && txt_serial.Text.Trim() != "N/A")
            {
                ...
                if (test.Rf == "Passed")
                    modeloorden.Tested = 1;
                else
                    modeloorden.Tested = 2;
        ...
        private void LoadForm()
        {
            ...
            //caja.Id_caja = int.Parse(caja.ReturnValue("select TOP 1 id_caja from tb_ModeloOrden where id_orden = " + orden.Id_orden + " order by id_modeloOrden desc"));
            ...
            if (lbl_ensamble.Text == "0")
                lbl_ensamble.Text = modeloorden.ReturnValue("select e.ensamble from tb_Orden o join tb_Ensamble e on o.id_ensamble = e.id_ensamble where id_orden = '" + orden.Id_orden + "'");
