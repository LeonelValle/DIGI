...
namespace MWTrace_beta
{
    public partial class NuevaOrden : Form
    {
        ...
        {
            DataTable dt = ensamble.LlenarComboBox("select id_ensamble, ensamble from tb_Ensamble");

            AutoCompleteStringCollection stringCol = new AutoCompleteStringCollection();

            foreach (DataRow row in dt.Rows)
            {
                stringCol.Add(Convert.ToString(row["ensamble"]));
            }
        ...
        private void Btn_aceptar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    orden.Crud("insert into tb_orden (orden, cantidad, fechaOrden, id_operador, id_ensamble ,rev) values('" + txt_orden.Text + "' , " + txt_cantidad.Text + " , '" + fecha.ToString("MM/dd/yyyy") + "' , " + cb_operador.SelectedValue + " , " + cb_ensamble.SelectedValue + ", '" + txt_revision.Text.ToUpper() + "')");
                    ///CAMBIAR///
