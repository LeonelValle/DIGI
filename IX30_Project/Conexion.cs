...
namespace IX30_Project
{
    class Conexion
    {
        ...
        Con1 = new SqlConnection(@"Data Source=MEX-SQL-001;Initial Catalog=DIGITrace;User ID=admin_sql;Password=DaBatase#40107;Connect Timeout=30;Encrypt=False;TrustServerCertificate=False;ApplicationIntent=ReadWrite;MultiSubnetFailover=False");
        //RESPARLDO
        //Con1 = new SqlConnection(@"Data Source=MEX-W10-9SANKBV\SQLEXPRESS;Initial Catalog=DIGITrace;Integrated Security=True;Connect Timeout=30;Encrypt=False;TrustServerCertificate=False;ApplicationIntent=ReadWrite;MultiSubnetFailover=False");
        ...
        public byte[] ReturnHex(string comando)
        {
            byte[] hex = new byte[4];
            Cerrar();
            using (SqlCommand cm = Con1.CreateCommand())
            {
                Abrir();
                cm.CommandText = comando;
                hex = cm.ExecuteScalar() as byte[];
                Cerrar();
                return hex;
            }
        }
