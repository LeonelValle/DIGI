        ...
        private void Btn_aceptar_Click(object sender, EventArgs e)
        {
            ...
            if (cb_filtro.Text == "Serial Number")
            {
                if (modeloorden.Existe("select COUNT(*) from tb_modeloOrden where serialNumber = '" + txt_buscar.Text + "'"))
                {
                    ...
                    //salida_datos = "select mo.id_modeloOrden as ID, moc.id_mocaja as IdCaja, e.ensamble, mo.Serialnumber, mo.scanmac, mo.scanimei, moc.SerialNumberCaja, moc.scanmacCaja, moc.scanimeiCaja, o.orden, mo.fecharegistro, c.caja, mo.turno, mo.revision, op.numeroempleado from tb_ModeloOrden mo join tb_Orden o on o.id_orden = mo.id_orden join tb_ModeloOrdenCaja moc on o.id_orden = moc.id_orden join tb_Ensamble e on e.id_ensamble = o.id_ensamble join tb_caja c on c.id_caja = mo.id_caja and c.id_caja = moc.id_caja join tb_Operador op on op.id_operador = o.id_operador and u.id_mocaja = moc.id_mocaja where mo.id_modeloOrden = '" + id_modelorden + "' order by mo.id_modeloOrden asc";
                }
                //else
                //{
                //    int id_mo = int.Parse(modeloorden.ReturnID("select id_mocaja from tb_ModeloOrdenCaja where SerialNumberCaja = '" + txt_buscar.Text + "'"));

                //    salida_datos = "select mo.id_modeloOrden as ID, e.ensamble, mo.Serialnumber, mo.scanmac, mo.scanimei, moc.SerialNumberCaja, moc.scanmacCaja, moc.scanimeiCaja, o.orden, mo.fecharegistro, c.caja, mo.turno, mo.revision, op.numeroempleado from tb_ModeloOrden mo join tb_Orden o on o.id_orden = mo.id_orden join tb_ModeloOrdenCaja moc on o.id_orden = moc.id_orden join tb_Ensamble e on e.id_ensamble = o.id_ensamble join tb_caja c on c.id_caja = mo.id_caja and c.id_caja = moc.id_caja join tb_Operador op on op.id_operador = o.id_operador and u.id_mocaja = moc.id_mocaja where moc.id_mocaja = '" + id_mo + "' order by mo.id_modeloOrden asc";
                //}
            ...
            if (cb_filtro.Text == "Caja")
            {
                ...
                //salida_datos = "select distinct mo.id_modeloOrden, e.ensamble, mo.Serialnumber, mo.scanmac, mo.scanimei, moc.SerialNumberCaja, moc.scanmacCaja, moc.scanimeiCaja, o.orden, mo.fecharegistro, c.caja, mo.turno, mo.revision, op.numeroempleado from tb_ModeloOrden mo join tb_Orden o on o.id_orden = mo.id_orden join tb_ModeloOrdenCaja moc on o.id_orden = moc.id_orden join tb_Ensamble e on e.id_ensamble = o.id_ensamble join tb_caja c on c.id_caja = mo.id_caja and c.id_caja = moc.id_caja join tb_Operador op on op.id_operador = o.id_operador where o.id_orden = " + id_orden + " order by mo.id_modeloOrden asc";
                salida_datos = "select distinct mo.id_modeloOrden as ID, e.ensamble, mo.Serialnumber, mo.scanmac, mo.scanimei, o.orden, mo.fecharegistro, c.caja, mo.turno, mo.revision, op.numeroempleado from tb_ModeloOrden mo join tb_Orden o on o.id_orden = mo.id_orden join tb_Ensamble e on e.id_ensamble = o.id_ensamble join tb_caja c on c.id_caja = mo.id_caja join tb_Operador op on op.id_operador = o.id_operador where mo.id_caja = " + id_caja + " order by mo.id_modeloOrden asc";

            }
            //salida_datos = "select distinct mo.id_modeloOrden as ID, moc.id_mocaja as IdCaja, e.ensamble, mo.Serialnumber, mo.scanmac, mo.scanimei, moc.SerialNumberCaja, moc.scanmacCaja, moc.scanimeiCaja, o.orden, mo.fecharegistro, c.caja, mo.turno, mo.revision, op.numeroempleado from tb_ModeloOrden mo join tb_Orden o on o.id_orden = mo.id_orden join tb_ModeloOrdenCaja moc on o.id_orden = moc.id_orden join tb_Ensamble e on e.id_ensamble = o.id_ensamble join tb_caja c on c.id_caja = mo.id_caja and c.id_caja = moc.id_caja join tb_Operador op on op.id_operador = o.id_operador and u.id_mocaja = moc.id_mocaja where c.caja = '" + txt_buscar.Text + "' order by mo.id_modeloOrden asc";

            if (cb_filtro.Text == "Ensamble")
            {
                //int id_ensamble = int.Parse(modeloorden.ReturnValue("select id_ensamble from tb_Ensamble where ensamble = '" + txt_buscar.Text + "'"));
                //salida_datos = "select distinct mo.id_modeloOrden, e.ensamble, mo.Serialnumber, mo.scanmac, mo.scanimei, moc.SerialNumberCaja, moc.scanmacCaja, moc.scanimeiCaja, o.orden, mo.fecharegistro, c.caja, mo.turno, mo.revision, op.numeroempleado from tb_ModeloOrden mo join tb_Orden o on o.id_orden = mo.id_orden join tb_ModeloOrdenCaja moc on o.id_orden = moc.id_orden join tb_Ensamble e on e.id_ensamble = o.id_ensamble join tb_caja c on c.id_caja = mo.id_caja and c.id_caja = moc.id_caja join tb_Operador op on op.id_operador = o.id_operador where o.id_ensamble = " + id_ensamble + " order by mo.id_modeloOrden asc";
            ...
            //dg_buscar.DataSource = modeloorden.BindGrid(salida_datos);
